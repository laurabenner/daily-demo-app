...
export function Popper({ popped, updatePopped, favorites, updateFavorites }) {
    ...
    return (
        <div className={(popped ? "block " : "hidden ") + "font-poppins w-1/2 h-auto bg-white rounded-lg shadow-lg p-6 absolute left-1/4 top-48 mx-auto grid"}>
            <h1 className="text-3xl text-asia-trail font-bold">Favorites</h1>
            <button onClick={handleClick} className="col-start-10 text-right px-2.5 float-right">
