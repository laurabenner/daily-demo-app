 ...
 */
export function AnimalDropdown({ animalSelected, updateAnimalSelected }) {
    ...
    return (
        <div className={"justify-self-end hidden md:grid w-11/12 grid-cols-10 p-4 border border-transparent rounded-b-big shadow-md text-white bg-palette-dark"}>
            <img className={"w-28 h-28 mr-2.5 col-start-1 col-span-2 row-span-3 justify-self-center border-4 rounded-full object-cover border-palette-light"} src={animalSelected.thumbnail} alt={animalSelected.label}></img>
            <p className="px-1 col-start-3 col-end-10 text-lg">
                <FontAwesomeIcon icon={faPaw} />
                {" " + animalSelected.label + " "}
                <i className="text-sm">{"(" + animalSelected.genus_and_species + ")"}</i>
            </p>
            ...
            <span className={"px-1 col-start-3 col-end-11 text-palette-light"}>
                ...
                {" Conservation Status: " + animalSelected.iucn}
            </span>
            <p className={"text-white px-1 col-start-3 col-end-11"}>
                {cutSentences(spliceTags(animalSelected.description))}
                <span className="inline-block text-palette-light">
                    <a
                        className="underline px-1"
                        href={animalSelected.path}
                        target="_blank"
                        rel="noreferrer">
                        Read more
                    </a>
